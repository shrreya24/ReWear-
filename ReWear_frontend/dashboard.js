...
document.addEventListener('DOMContentLoaded', loadSwapRequests);

function loadSwapRequests() {
  const token = localStorage.getItem("userToken");
  fetch("http://localhost:5000/api/swap/incoming", {
    headers: { Authorization: `Bearer ${token}` }
  })
  .then(res => res.json())
  .then(renderSwapRequests)
  .catch(() => {
    document.getElementById("swap-requests-list").innerHTML = "<p>Error loading requests</p>";
  });
}
...
function renderSwapRequests(requests) {
  ...
  requests.forEach(req => {
    ...
    card.innerHTML = `
      ...
      ${req.status === "pending" ? `
      <div class="request-buttons">
        <button class="accept-btn" onclick="respondToRequest('${req._id}', 'accepted')">Accept</button>
        <button class="reject-btn" onclick="respondToRequest('${req._id}', 'rejected')">Reject</button>
        <button class="view-items-btn" onclick="viewRequesterItems('${req.requesterId._id}')">View Items</button>
      </div>` : ""}
    `;
...
function respondToRequest(requestId, decision) {
  ...
  fetch("http://localhost:5000/api/swap/respond", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
// ✅ View Requester's Items
function viewRequesterItems(userId) {
  const token = localStorage.getItem("userToken");
  const modal = document.getElementById('view-items-modal');
  const body = document.getElementById('requester-items-body');

  modal.classList.add('active');
  body.innerHTML = "<p>Loading...</p>";

  fetch(`http://localhost:5000/api/user/${userId}/items`, {
    headers: { Authorization: `Bearer ${token}` }
  })
  .then(res => res.json())
  .then(items => {
    if (!items.length) {
      body.innerHTML = "<p>This user has no items listed.</p>";
      return;
    }
    ...
    body.innerHTML = items.map(item => `
      <div style="border:1px solid #ddd; border-radius:10px; padding:1rem; margin-bottom:1rem;">
        ...
        <p><strong>Years Used:</strong> ${item.yearsUsed || 'N/A'}</p>
        ${item.imageUrl ? `<img src="/uploads/${item.imageUrl}" style="width:100%; max-width:200px; margin-top:10px;">` : ""}
      </div>
    `).join('');
  })
  .catch(() => {
    body.innerHTML = "<p>Error fetching items.</p>";
  });
}

function closeItemsModal() {
  document.getElementById('view-items-modal').classList.remove('active');
}
...
document.getElementById('uploadForm').addEventListener('submit', function (e) {
  ...
  fetch('http://localhost:5000/api/items/upload', {
    method: 'POST',
    headers: { Authorization: `Bearer ${token}` },
    body: formData
  })
